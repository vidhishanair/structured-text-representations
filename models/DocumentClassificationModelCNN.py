...
class DocumentClassificationModelCNN(nn.Module):
    def __init__(self, device, vocab_size, token_emb_size, sent_hidden_size, doc_hidden_size, sent_num_layers, doc_num_layers, sem_dim_size, pretrained=None, dropout=0.5, bidirectional=True, py_version='nightly'):
        super(DocumentClassificationModelCNN, self).__init__()
        self.device = device
        ...
        self.sentence_encoder = CNNEncoder(device, self.sent_hidden_size, token_emb_size, sent_num_layers, dropout, bidirectional)
        self.document_encoder = CNNEncoder(device, self.doc_hidden_size, self.sem_dim_size, doc_num_layers, dropout, bidirectional)
