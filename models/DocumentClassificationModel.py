...
#         print("--- %s seconds for Structured Attention document ---" % (time.time() - start_time)) 
        ...
        if torch.sum(torch.isnan(encoded_documents))>=1:
            print("Documents before drop:")
            print(encoded_documents)
        #encoded_documents = self.drop(encoded_documents)
        #encoded_documents = self.sentence_structure_att.LReLu(self.pre_lin1(encoded_documents))
        #encoded_documents = self.drop(encoded_documents)
        #encoded_documents = self.sentence_structure_att.LReLu(self.pre_lin2(encoded_documents))
        #Linear for output
